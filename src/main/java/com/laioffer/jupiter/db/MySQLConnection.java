...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance(); // bypass error in JDBC implementation
            conn = DriverManager.getConnection(MySQLDBUtil.getMySQLAddress()); // create connection
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void close() {
        if (conn != null) {
            try {
                conn.close();
                // 1. the number of connections allowed for a db is limited
                //    it is a waste of resource if we don't close
                // 2. open a connection has cost locally in memory, if we close it
                //    we can have more memory to use for later operations
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // insert into favorite table, and because of foreign key, we need to check if Item exist in item table
        ...
        // Safer because can prevent injection attack
        try {
            // IGNORE: ignore failure on insertion
    ...
    public void unsetFavoriteItem(String userId, String itemId) throws MySQLException {
        ...
        // do we need to delete item from item_table immediately?
        // 1. delete immediately only possible when the current user is the only one who liked the item
        // 2. delete such unrefered item periodically, so that the response time from user point of view can be shorter
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, userId);
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            // return value checks whether we inserted 1 row successfully (check function return value)
